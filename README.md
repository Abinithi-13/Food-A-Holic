...
Here’s how you can call the Grok API inside your React app:

```javascript
// src/services/grokApi.js
export async function generateRecipe(ingredients) {
  const apiKey = process.env.REACT_APP_GROK_API_KEY;

  const response = await fetch("https://api.grok.com/v1/recipes", {
    method: "POST",
    headers: {
...
```javascript
import { generateRecipe } from "./services/grokApi";

async function handleGenerate() {
  const recipe = await generateRecipe("tomatoes, garlic, basil");
  setRecipe(recipe);
}
```
